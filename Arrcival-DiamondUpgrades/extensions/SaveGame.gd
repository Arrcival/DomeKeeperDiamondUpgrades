...
const SAVE_META4_FILE_TEMPLATE_DIAMOND := "user://savegame4_diamond_%s.json"
const SAVE_META4_FILE_TEMPLATE_DIAMOND_BACKUP := "user://savegame4_diamond_%s_backup_%s.json"

func create_metadata_backup():
	var id = "full"
	var saveFile: String= SAVE_META4_FILE_TEMPLATE_DIAMOND % id
	var time = Time.get_datetime_dict_from_system()
	var timestr = "%s_%s_%s_%s_%s" %[time.year, time.month, time.day, time.hour, time.minute]
	var d := DirAccess.open("user://")
	if d.file_exists(saveFile):
		d.copy(saveFile, SAVE_META4_FILE_TEMPLATE_DIAMOND_BACKUP % [id, timestr])
		
func deleteMetaState():
	var saveFile = SAVE_META4_FILE_TEMPLATE_DIAMOND % CONST.BUILD_TYPE.keys()[GameWorld.buildType].to_lower()
...
func loadMetaStateFile(slot:int = 0) -> Array:
	...
	if slot == -1:
		saveFile = SAVE_META4_FILE_TEMPLATE_DIAMOND % CONST.BUILD_TYPE.keys()[GameWorld.buildType].to_lower()
		...
		saveFile = SAVE_META4_FILE_TEMPLATE_DIAMOND % file_str
		if !FileAccess.file_exists(saveFile):
...
func persistMetaState(slot:int = 0):
	var file_str : String = str(CONST.BUILD_TYPE.keys()[GameWorld.buildType].to_lower(), str(slot))
	var saveFile = SAVE_META4_FILE_TEMPLATE_DIAMOND % file_str
